...
// function Favorites() {
//     // const [favsName, setFavsName] = useState([])
//     const favorites = useSelector((state) => state.favList.favorites);
//     const dispatch = useDispatch();
...
//             <input placeholder="New list name: " className="favorites__name"/>
//             <ul className="favorites__list">
//                 {favorites.map(f => <li key={f.id} className="listItem"> {f.title}  {f.year}  <button type="button" className="delBtn" onClick={() => { dispatch(removeFromFavList({ id: f.id })) }}> X </button></li>)}
